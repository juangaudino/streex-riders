-- STREEX Rides availability system, Phase 4.1.
-- Apply this to the current production Supabase project after the clean baseline.
-- This is intentionally Google-Calendar-free; Google sync is a later phase.
...
begin
  if not exists (
    select 1
    from pg_constraint
    where conname = 'bookings_estimated_duration_minutes_check'
  ) then
    alter table public.bookings
